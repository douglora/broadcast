...
def agenda(calendario: dict, hoje: date, dias: int = 6, so_confianca: tuple = ("alta", "media"), extras: list | None = None,
    ...
    partir de 24/09 com a janela unica de 6 dias. O fechamento lista de amanha em
    diante; a manha (`incluir_hoje`) comeca no proprio dia: em 24/09 o RPM das 09:00
    e o leilao das 11:00 ficaram de fora do card."""
    fim = hoje + timedelta(days=dias)
    fim_emp = hoje + timedelta(days=dias_empresas)
    primeiro = hoje if incluir_hoje else hoje + timedelta(days=1)
    e_empresa = lambda t: (" resultado " in t) or (" ex-dividendo " in t)
    itens = [(d, t) for d, t in (extras or []) if primeiro <= d <= (fim_emp if e_empresa(t) else fim)]
